...
- `gemini_secret_arn` - Secrets Manager ARN for the OpenAI API key
- `user_credentials` - (sensitive) IAM access keys per user

---

## Step 6: Store the OpenAI API Key in Secrets Manager

Terraform creates the secret with a placeholder. Replace it with your real key:

```bash
OPENAI_SECRET_ARN=$(cd infra && terraform output -raw gemini_secret_arn)

aws secretsmanager put-secret-value \
  --secret-id "${OPENAI_SECRET_ARN}" \
  --secret-string "sk-...your-real-key..." \
  --region us-east-1

Note: the Batch job definition injects this secret into the container as `GEMINI_API_KEY`,
and the entrypoint maps it to `OPENAI_API_KEY` for OpenEvolve.
```
...
User
 ...
 |--> AWS Batch Job Queue
         ...
         +--> Docker Container (openevolve:latest from ECR)
                 ...
                 |       LLM: OpenAI (via OpenAI-compatible API)
...
## Combined Architecture (CPU + Optional GPU Evaluators)
...
User
 ...
 |--> AWS Batch Job Queue
         |
         +--> CPU Compute Environment (Spot-first, On-Demand fallback)
         |       EC2: m5/c5/r5 instances
         |       Runs OpenEvolve controller container
         |         - Builds prompts
         |         - Calls LLM API
         |         - Schedules evaluations
         |
         +--> GPU Compute Environment (optional)
         |       EC2: g5/g4dn instances
         |       Runs evaluator jobs (e.g., LLM training)
         |
         +--> Docker Container (openevolve:latest from ECR)
                 |
                 |--> Downloads inputs from S3
                 ...
                 |--> Runs OpenEvolve evolution loop
                 |--> Syncs checkpoints to S3 every 5 min
                 |--> Uploads final results to S3 Output Bucket
                 |
                 +--> CloudWatch Logs (/aws/batch/openevolve)
                 +--> CloudWatch Metrics (OpenEvolve namespace)
...
- **S3 Input Bucket**: holds the initial program, evaluator, and optional config that the Batch job downloads.
- **AWS Batch Queue**: schedules work onto either CPU or GPU compute environments.
- **CPU Compute Environment**: runs the OpenEvolve controller (prompting, sampling, and orchestration).
- **GPU Compute Environment (optional)**: used only if your evaluator trains models or otherwise needs GPU.
- **ECR image + entrypoint**: downloads inputs, runs the evolution loop, checkpoints to S3, and uploads outputs.
- **S3 Output Bucket**: stores checkpoints, logs, and the best evolved program.
- **CloudWatch + SNS**: central logging, metrics, and alerts.

If you do not need GPU evaluators, you can omit the GPU compute environment and run everything on CPU.
...
Common causes:
- OpenAI API key not set in Secrets Manager (Step 6)
